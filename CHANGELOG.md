...
- Version in statusbar now read from package.json instead of hardcoded
...
- Theme cascade: Omarchy `colors.toml` → `config.json` `theme` section → Tokyo Night defaults
...
- List position preserved after sync toggle instead of jumping to top
- Navigate up (`u`) now selects the folder you came from
